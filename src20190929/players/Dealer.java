...
import hands.DealerHand;
import hands.PlayerHand;
			...
			default:
				...
				switch(dealerHand.getStatus()) {
					...
					default:
						...
						int playerValue = (Math.max(val[0], val[1]) > 21) ? Math.min(val[0], val[1]) : Math.max(val[0], val[1]);
						val = dealerHand.getValue();
						int dealerValue = (Math.max(val[0], val[1]) > 21) ? Math.min(val[0], val[1]) : Math.max(val[0], val[1]);
