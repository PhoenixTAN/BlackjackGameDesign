...
public class CommonPlayer extends BlackjackPlayer {
	...
	public boolean showAvailableActions( int whichHand ) {
		...
		switch(sta) {
			/*case SPLIT:
				...
				break;*/
			case FOLD:
				System.out.println("Smart choice to give up!");
				break;
			case DEFAULT:
				hasNextAction = true;
	...
	public int inputActions(int whichHand) {
		...
		try{
	         ...
	         System.out.println("Choose 1 for you!");
	    }
	...
	public void doubleUp(Card c, int whichHand) {
		...
		if( this.cash < this.playerHand.get(whichHand).getStake() ) {
			System.out.println("You don't have enough money to double. Just hit for you.");			
			this.receiveCards(c, whichHand);
			this.playerHand.get(whichHand).printValue();
			return ;
		}
	...
	/**
	 * Method: betOrFold
	 * In trianta Ena, the player can choose bet or fold,
	 * right after receiving the first card.
	 * */
	public int betOrFold() {
		Scanner scan = new Scanner(System.in);
		int choice = 2;
		try{
	         choice = scan.nextInt();	
	         ...
	         System.out.println("Exception thrown  :" + e);
	         System.out.println("Choose 2 for you!");
	    }
